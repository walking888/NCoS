...
decode_nxast_action(const union ofp_action *a, enum ofputil_action_code *code)
{
    ...
    if (len < sizeof(struct nx_action_header)) {
        return OFPERR_OFPBAC_BAD_LEN;
/* by Liu Sicheng */
	} else if (a->vendor.vendor == CONSTANT_HTONL(NC_VENDOR_ID)) {
		return decode_nc_action(a, code);
/* End by Liu Sicheng */
...
/* by Liu Sicheng */
static enum ofperr
nc_init_coding_from_openflow(const struct nc_action_init_coding *nca,
	struct ofpbuf *out)
{
	unsigned int data_len;
	char *p;
	int i;
	struct ofpact_init_coding *s = ofpact_put_INIT_CODING(out);
	s->buffer_id = nca->buffer_id;
	s->packet_num = nca->packet_num;
	s->port_num = nca->port_num;
	s->vector_off = nca->vector_off;
	s->packet_len = ntohs(nca->packet_len);
	data_len = nca->port_num * 2 + nca->port_num * nca->packet_num * 1;
	ofpbuf_put(out, nca->dataasdf, data_len);
	s->ofpact.len += data_len;
	return 0;
}

static enum ofperr
nc_encode_from_openflow(const struct nc_action_encode *nca,
	struct ofpbuf *out)
{
	unsigned int data_len;
	char *p;
	int i;
	struct ofpact_encode *s = ofpact_put_ENCODE(out);
	s->buffer_id = nca->buffer_id;
	s->port_num = nca->port_num;
	s->buffer_size = ntohs(nca->buffer_size);
	s->output_port = ntohs(nca->output_port);
	s->packet_len = ntohs(nca->packet_len);
	s->packet_num = ntohs(nca->packet_num);
	s->port_id = ntohs(nca->port_id);
	data_len = ntohs(nca->packet_num) * 1;
	ofpbuf_put(out, nca->dataasdf, data_len);
	s->ofpact.len += data_len;
	return 0;
}
...
static enum ofperr
nc_decode_from_openflow(const struct nc_action_decode *nca,
	struct ofpbuf *out)
{
	unsigned int data_len;
	char *p;
	int i;
	struct ofpact_decode *s = ofpact_put_DECODE(out);
	s->buffer_id = nca->buffer_id;
	s->packet_num = nca->packet_num;
	s->buffer_size = ntohs(nca->buffer_size);
	s->output_num = ntohs(nca->output_num);
	s->packet_len = ntohs(nca->packet_len);
	s->port_id = ntohs(nca->port_id);
	data_len = ntohs(nca->output_num) * 2;
	ofpbuf_put(out, nca->dataasdf, data_len);
	s->ofpact.len += data_len;
	return 0;
}
...
static enum ofperr
ofpact_from_nc(const union ofp_action *a, enum ofputil_action_code code,
			struct ofpbuf *out)
{
	enum ofperr error = 0;
	switch(code) {
	case OFPUTIL_NC_INIT_CODING:
		error = nc_init_coding_from_openflow((const struct nc_action_init_coding *)a, out);
		break;
	case OFPUTIL_NC_ENCODE:
		error = nc_encode_from_openflow((const struct nc_action_encode *)a, out);
		break;
	case OFPUTIL_NC_DECODE:
		error = nc_decode_from_openflow((const struct nc_action_decode *)a, out);
		break;
	}
	return error;
}
...
#include "ofp-util.def"
    ...
    case OFPUTIL_OFPAT10_ENQUEUE:
        ...
        break;
/* by Liu Sicheng */
#define NC_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME) case OFPUTIL_##ENUM:
#include "ofp-util.def"
	return ofpact_from_nc(a, code, out);
/* End by Liu Sicheng */

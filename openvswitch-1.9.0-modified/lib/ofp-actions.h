...
#define OFPACTS                                                     \
    ...
    DEFINE_OFPACT(CLEAR_ACTIONS,   ofpact_null,          ofpact)    \
    DEFINE_OFPACT(GOTO_TABLE,      ofpact_goto_table,    ofpact)    \
	DEFINE_OFPACT(INIT_CODING,	ofpact_init_coding,	dataasdf)	\
	DEFINE_OFPACT(ENCODE,	ofpact_encode,	dataasdf)	\
	DEFINE_OFPACT(DECODE,	ofpact_decode,	dataasdf)	
...
/* by Liu Sicheng */
/* the smae as openvswitch.h but add some head */
struct ofpact_init_coding {
	struct ofpact ofpact;
	uint8_t buffer_id;
	uint8_t packet_num;
	uint8_t port_num;

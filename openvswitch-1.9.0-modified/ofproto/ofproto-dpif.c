...
/* by Liu Sicheng */
static void
xlate_nc_init_coding_action(struct action_xlate_ctx *ctx,
		const struct ofpact *a)
{
	nl_msg_put_unspec(ctx->odp_actions, OVS_ACTION_ATTR_INIT_CODING,
		a+1, a->len - sizeof(struct ofpact));
}
static void
xlate_nc_encode_action(struct action_xlate_ctx *ctx,
		const struct ofpact *a)
{
	nl_msg_put_unspec(ctx->odp_actions, OVS_ACTION_ATTR_ENCODE,
		a+1, a->len - sizeof(struct ofpact));
}
static void
xlate_nc_decode_action(struct action_xlate_ctx *ctx,
		const struct ofpact *a)
{
	nl_msg_put_unspec(ctx->odp_actions, OVS_ACTION_ATTR_DECODE,
		a+1, a->len - sizeof(struct ofpact));
}
/* End by Liu Sicheng */
...
/* by Liu Sicheng */
		case OFPACT_INIT_CODING:
			xlate_nc_init_coding_action(ctx, a);
			break;
		case OFPACT_ENCODE:
			xlate_nc_encode_action(ctx, a);
			break;
		case OFPACT_DECODE:
			xlate_nc_decode_action(ctx, a);
			break;
/* End by Liu Sicheng */
        }
    }

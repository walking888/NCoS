...
/* by Liu Sicheng */
	nla_for_each_nested(a, attr, rem) {
		...
		static const u32 action_lens[OVS_ACTION_ATTR_MAX + 1] = {
			...
			[OVS_ACTION_ATTR_INIT_CODING] = (u32) - 1,
			[OVS_ACTION_ATTR_ENCODE] = (u32) - 1,
			[OVS_ACTION_ATTR_DECODE] = (u32) - 1
		};
/* End by Liu Sicheng */
...
static int ovs_packet_cmd_execute(struct sk_buff *skb, struct genl_info *info)
{
	...
	err = -ENODEV;
/* by Liu Sicheng */
   // printk(KERN_INFO "@@ovs_packet_cmd_execute at %p\n", dp);
/* End by Liu Sicheng */
	if (!dp)
		goto err_unlock;
...
static int ovs_flow_cmd_new_or_set(struct sk_buff *skb, struct genl_info *info)
{
	...
	error = -ENODEV;
/* by Liu Sicheng */
   // printk(KERN_INFO "@@ovs_flow_cmd_new_or_set at %p\n", dp);
/* End by Liu Sicheng */
	if (!dp)
		goto err_kfree;
...
#define AUTO_ADAPT_BUFFER_SIZE 0xffff
static void ovs_clean_buffer_for_reuse(struct datapath *dp, \
        uint8_t x, uint8_t y, uint16_t z)
{
    struct __liu_buffer * buff = &((dp->buffer).buff[x]);
    unsigned int tmp = buff->tail;

    if (z == AUTO_ADAPT_BUFFER_SIZE)
        z = buff->max_buffer;

    while (tmp != buff->head[y]){
        if(buff->buffer[y][tmp] != NULL) {
            kfree_skb(buff->buffer[y][tmp]);
            buff->buffer[y][tmp] = NULL;
            buff->mMatrix[y][tmp] = NULL;
            buff->flag[tmp] = 0;
        }
        buff->generation_id[tmp] = 0; 

        tmp = add_round(tmp, z);
    }

    buff->head[y] = buff->tail;

    // for debug
    printk(KERN_INFO "!!clean buffer for reuse at %p, %d, %d\n", dp, x, y);
}
static void ovs_buffer_clean_while_remove_flow(struct datapath *dp, struct sw_flow *flow)
{
    ...
    for(a = acts->actions, rem = acts->actions_len; 
            rem > 0 ; a = nla_next(a, &rem)) {
        x = 255;
        switch(nla_type(a)) {
            case OVS_ACTION_ATTR_OUTPUT:
            case OVS_ACTION_ATTR_USERSPACE:
            ...
            case OVS_ACTION_ATTR_SAMPLE:
                break;
            case OVS_ACTION_ATTR_INIT_CODING:
                x = (((struct ovs_action_init_coding *) nla_data(a))->buffer_id) & 0x7f;
                y = ((struct ovs_action_init_coding *) nla_data(a))->vector_off;
                z = AUTO_ADAPT_BUFFER_SIZE; 
                break;
            case OVS_ACTION_ATTR_ENCODE:
                x = ((struct ovs_action_encode *) nla_data(a))->buffer_id;
                y = ((struct ovs_action_encode *) nla_data(a))->port_id;
                z = ((struct ovs_action_encode *) nla_data(a))->buffer_size;
                break;
            case OVS_ACTION_ATTR_DECODE:
                x = ((struct ovs_action_decode *) nla_data(a))->buffer_id;
                y = ((struct ovs_action_decode *) nla_data(a))->port_id;
                z = ((struct ovs_action_decode *) nla_data(a))->buffer_size;
                break;
        }
        if(x != 255) {
            // need to clean for reuse buffer
            ovs_clean_buffer_for_reuse(dp, x, y, z); 
        }
    }
}
/* End by Liu Sicheng */
...
/* by Liu Sicheng */
    // for clean the buffer when remove flow 
    ovs_buffer_clean_while_remove_flow(dp, flow);
/* End by Liu Sicheng */
...
/* by Liu Sicheng */
    printk(KERN_INFO "init for NC, size %d\n", MAX_CB_SIZE);
    ovs_liu_buffer_init(&(dp->buffer));
    //here use static location
    initMulDivTab("/home/lsch/mytest/openvswitch-1.9.0/datapath/muldiv.tab");
    for(i = 0; i < MAX_LIU_BUFFER; i++)
        ovs__liu_buffer_init(&(dp->buffer.buff[i]), MAX_INPORT_NUM, MAX_PACKET_NUM);
/* End by Liu Sicheng */
	rtnl_unlock();
...
/* by Liu Sicheng */
    //clean buffer
    for (i = 0; i < MAX_LIU_BUFFER; i++) {
        ovs__liu_buffer_free(&(dp->buffer.buff[i]));
    }
/* End by Liu Sicheng */
